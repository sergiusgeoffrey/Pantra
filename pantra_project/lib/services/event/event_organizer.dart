...
class EventOrganizerService {
  Future<List<StringObj>> getAllData() async {
    ...
    final response = await http.get(
        Uri.parse(
            "https://bem-internal.petra.ac.id/reach/api/event/organizer/index.php"),
        headers: {
    ...
    if (response.statusCode == 200) {
      ...
      List<StringObj> eventOrganizers = [];
      for (var i = 0; i < jsonData['data'].length; i++) {
        eventOrganizers.add(StringObj(
          data: jsonData['data'][i]['organizer'],
        ));
      }
